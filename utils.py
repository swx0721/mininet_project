...
def print_subnet_info():
    """打印子网划分与拓扑结构信息。"""
    info("差异化子网划分方案 (接入-汇聚二级交换)：\n")
    info("  - 宿舍区: 10.0.0.0/20  (约 4,094 可用地址) — 大量终端接入\n")
    info("    * 接入层: s_dorm1/2/3 → 汇聚层: s_dorm_agg → r1-eth0\n")
    info("  - 教学楼: 10.0.16.0/20 (约 4,094 可用地址) — 大量终端接入\n")
    info("    * 接入层: s_teach1/2/3 → 汇聚层: s_teach_agg → r1-eth1\n")
    info("  - 图书馆: 10.0.32.0/23 (约   510 可用地址) — 中等终端密度 (单交换机直连 r1-eth2)\n")
    info("  - 办公楼: 10.0.34.0/24 (约   254 可用地址) — 中等终端密度 (单交换机直连 r1-eth3)\n")
    info("  - 财务处: 10.0.35.0/26 (约    62 可用地址) — 少量敏感终端 (单交换机直连 r1-eth4)\n")
    info("  - 人事处: 10.0.35.64/26 (约    62 可用地址) — 少量人事终端 (单交换机直连 r1-eth7)\n")
    info("  - 服务器: 10.0.60.0/28 (约  14 可用地址) — 独立服务器区 (与业务区地址隔离)\n")
    info("    * server1 (10.0.60.2) 通过 s_server1 接入 r1-eth5\n")
    info("    * server2 (10.0.60.18) 通过 s_server2 接入 r1-eth6（独立子网，对称双链路）\n")


def print_service_help(server_ip="10.0.60.2"):
    """打印可用服务测试命令。"""
    info("可用服务测试命令：\n")
